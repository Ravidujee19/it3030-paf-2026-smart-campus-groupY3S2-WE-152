...
public class AnalyticsService {
     ...
     * @return Map where key is DayOfWeek (1-7) and value is an array of 24 hourly intensities.
     */
    public Map<String, List<Long>> getBookingHeatmap() {
        ...
        grid.forEach((day, hours) -> {
            ...
            for (long h : hours) hourList.add(h);
    ...
    public Map<String, Double> getCapacityEfficiency() {
    ...
    public List<Map<String, String>> getSmartInsights() {
        List<Map<String, String>> insights = new ArrayList<>();
        List<Booking> bookings = bookingRepository.findAll();
        
        if (bookings.isEmpty()) return insights;
        ...
        if (peakHours.get(maxHour) > 5) {
            insights.add(Map.of(
                "type", "CAPACITY",
                "message", String.format("High demand detected around %02d:00. Recommend load balancing scheduled sessions.", maxHour),
                "severity", "WARNING"
            ));
        }
        ...
        resourceCounts.entrySet().stream()
                ...
                .ifPresent(e -> insights.add(Map.of(
                    "type", "OPTIMIZATION",
                    "message", String.format("Resource '%s' has low utilization. Consider repurposing or maintenance.", e.getKey()),
                    "severity", "INFO"
                )));
    ...
    public Map<String, Object> getDashboardStats() {
        ...
        long active = allBookings.stream().filter(b -> b.getStatus() == Booking.BookingStatus.APPROVED || b.getStatus() == Booking.BookingStatus.CONFIRMED).count();
        ...
        // Calculate a simple day-over-day trend (dummy logic for visual demo, would use timestamps in prod)
