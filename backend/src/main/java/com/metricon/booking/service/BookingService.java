...
public class BookingService {
    ...
    public BookingDto createBooking(BookingDto dto, Long userId) {
        ...
        BookingDto result = mapToDto(bookingRepository.save(booking));
        ...
        java.util.List<User> admins = userRepository.findByRoleName(com.metricon.common.enums.RoleName.ADMIN);
        for (User admin : admins) {
            notificationService.createNotification(
                    admin.getEmail(),
                    "New Booking Request",
                    "User " + user.getName() + " requested to book \"" + resource.getName() + "\" from " 
                    + dto.getStartTime().toLocalDate() + ". Please review it.",
    ...
    public BookingDto reviewBooking(Long bookingId, Booking.BookingStatus newStatus, String reason) {
        ...
        String resourceName = booking.getResource() != null ? booking.getResource().getName() : "the resource";
        ...
        if (newStatus == Booking.BookingStatus.APPROVED) {
            notificationService.createNotification(
                    userEmail,
                    "Booking Approved ✓",
                    "Your booking for \"" + resourceName + "\" has been approved. You're all set!",
    ...
    public BookingDto completeBooking(Long bookingId) {
        ...
        if (booking.getStatus() != Booking.BookingStatus.CONFIRMED) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Only confirmed bookings can be completed.");
        }
